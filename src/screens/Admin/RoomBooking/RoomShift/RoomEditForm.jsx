...
function RoomEditForm({ setOpen, changedata }) {
  const [dharamshalid, setdharamshalid] = useState('');
  const [dateTime, setDateTime] = useState(getCurrentDateTime());
  const [coutdateTime, setCoutDateTime] = useState(getCurrentDateTime());
  ...
  useEffect(() => {
    ...
    if (changedata) {
      ...
      if (changedata?.date) {
        const date = new Date(changedata.date);
        const istDate = new Date(date.getTime() + (5.5 * 60 * 60 * 1000)); // Convert to IST
        setDateTime(istDate.toISOString().slice(0, 16));
      }
      
      if (changedata?.coutDate) {
        const coutDate = new Date(changedata.coutDate);
        const istCoutDate = new Date(coutDate.getTime() + (5.5 * 60 * 60 * 1000)); // Convert to IST
        setCoutDateTime(istCoutDate.toISOString().slice(0, 16));
      }
                ...
                <div className="form-input-div_add_user">
                  ...
                  <div className="inner-input-div2">
                    ...
                    <input
                        ...
                        padding: '0.6rem', // Add padding for better UX
                ...
                <div className="form-input-div_add_user">
                  ...
                  <div className="inner-input-div2">
                    ...
                    <input
                        ...
                        name="checkin-time"
                        value={coutdateTime}
                        onChange={(e) => setCoutDateTime(e.target.value)}
                        style={{
                        width: '100%', // Full width
                        padding: '0.6rem', // Add padding for better UX
                        border: '1px solid #ccc', // Light border
