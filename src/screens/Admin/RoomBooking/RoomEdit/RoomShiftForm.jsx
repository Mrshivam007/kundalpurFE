...
function RoomShiftForm({ setOpen, changedata }) {
  ...
  const handlesubmit = async () => {
    try {
      ...
      // Parse roomnumber into an array of strings
      const roomNumbersArray = roomnumber
        .split(',')
      ...
      const data = {
        ...
        roomList: roomNumbersArray, // Use array of strings here
  ...
  useEffect(() => {
    ...
    if (changedata) {
      ...
      setroomnumber(changedata?.roomNumbers?.join(',') || ''); // Convert array to comma-separated string
  ...
  const handleRoomNumberChange = (e) => {
    ...
    // Split by commas and filter out invalid values
    const arrayValue = inputValue
      .split(',')
      .map((item) => item.trim())
      .filter((item) => !isNaN(item) && item !== '') // Ensure only numeric values
      .map(Number); // Convert to numbers
    setroomnumber(arrayValue.join(',')); // Update the displayed value
    // If needed, update another state or send this array to a parent component
    console.log('Updated Room Numbers Array:', arrayValue);
  };
            ...
            <div className="cash-donation-container-innser10">
              <div className="form-div" style={{ marginBottom: '1rem' }}>
                <div className="form-input-div_add_user">
                  ...
                  <div className="inner-input-div2">
                    <label style={{ marginBottom: '0.3rem' }} htmlFor="roomnumber">
                      Room No
                    </label>
                    <CustomInput
                      id="roomnumber"
                      name="roomnumber"
                      placeholder="Enter Room Numbers (e.g., 8,9)"
                      value={roomnumber}
                      onChange={handleRoomNumberChange} // Handle changes
                    />
                  </div>
