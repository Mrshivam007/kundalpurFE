...
import { Button, DialogContent, DialogContentText, DialogTitle, Fade, Modal, Tooltip, DialogActions, Box, Typography, IconButton, Dialog } from "@mui/material";
import CloseIcon from "@mui/icons-material/Close";
import Update from "../Add/Update"

export default function Table({ department }) {
  ...
  const handleDelete = async () => {
    try {
      const response = await serverInstance(`store/delete-departmentMaster?id=${deleteId}`, "delete");
      if (response.status) {
        setOpen3(false);
                ...
                <tr key={dep.id}>
                  ...
                  <td>
                    ...
                    <Tooltip title="Delete">
                      <img
                        onClick={() => handleClickOpen3(dep.id)}
                        src={Delete}
                        alt="Delete"
                        style={{ width: "20px" }}
                      />
